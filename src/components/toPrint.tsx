...
export default function ToPrint({ selectedInvoice }) {
    ...
    <div className="view-invoices fixed  times left-0 top-0 z-50   w-full h-[870px] flex justify-center">
      ...
      <div className="cadre relative w-[560px] h-full bg-white pt-4">
        ...
        <div className="client flex items-start justify-between px-6 pt-2">
          <div className="idt flex flex-col">
            <div className="list flex items-center gap-4">
              ...
              <input type="text" defaultValue={`${selectedInvoice?.client?.firstName} ${selectedInvoice?.client?.name} `} className="outline-none text-sm" style={{ fontSize: "10px" }} />
            </div>
            <div className="list flex items-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>Adresse   <span>:</span></h3>
              <input type="text" defaultValue={selectedInvoice?.client?.adresse} className="outline-none text-sm w-64" style={{ fontSize: "10px" }} />
            </div>
            <div className="list flex items-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>Téléphone   <span>:</span></h3>
              <input type="text" defaultValue={selectedInvoice?.client?.telephone} className="outline-none text-sm" style={{ fontSize: "10px" }} />
            </div>
            <div className="list flex items-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>NIF   <span>:</span></h3>
              <input type="text" defaultValue={selectedInvoice?.client?.nif} className="outline-none text-sm" style={{ fontSize: "10px" }} />
            </div>
            <div className="list flex items-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>STAT   <span>:</span></h3>
              <input type="text" defaultValue={selectedInvoice?.client?.stat} className="outline-none text-sm" style={{ fontSize: "10px" }} />
            </div>
          </div>
          <div className="fact">
            <h3 className=" font-bold text-sm" style={{ fontSize: "13px" }}>Date:  {(format(new Date(), "dd/MM/yyyy"))}</h3>
            <div className="numero-facture border-2 border-black p-1 w-[190px] mt-1">
              ...
              <input type="text" className="outline-none mt-1 text-sm text-center font-bold w-full" defaultValue={""} placeholder="N facture" style={{ fontSize: "12px" }} />
            </div>
          </div>
        </div>
            ...
            <tbody>
              {
                ...
                <tr >
                  <td className="border h-3 border-l-2  border-black text-xs uppercase font-bold" style={{fontSize:"11px"}}>{selectedInvoice?.designation}</td>
                  <td className="border h-3 border-black text-center font-bold" style={{ fontSize: "11px" }}>{formatNumber(selectedInvoice?.quantite)}</td>
                  <td className="border h-3 text-end pr-1 border-black font-bold" style={{ fontSize: "11px" }}>
                    {formatNumber(selectedInvoice?.prix_ht / selectedInvoice?.quantite) || 0}   Ar</td>
                  <td className="border border-r-2 h-3  text-end pr-1 border-black font-bold" style={{ fontSize: "11px" }}>{formatNumber(selectedInvoice?.total_ht) || 0} Ar</td>
                </tr>
              }
              {
              ...
              <div className="" style={{ width: "130px", borderBottom: "1px double black", paddingBottom: "2px" }}>
                <div className=" " style={{ borderBottom: "1px double black" }}>
                  <h4 className="text-black text-end font-semibold" style={{ fontSize: "12px" }}>
                    {formatNumber(selectedInvoice?.total_ht) || 0}       Ar</h4>
                </div>
              </div>
            </div>
            <div className="flex">
              ...
              <div className="" style={{ width: "130px", borderBottom: "1px double black", paddingBottom: "2px" }}>
                <div className=" " style={{ borderBottom: "1px double black" }}>
                  <h4 className="text-black text-end font-semibold" style={{ fontSize: "12px" }}>
                    {formatNumber(selectedInvoice?.tva) || 0}   Ar</h4>
                </div>
              </div>
            </div>
            <div className="flex">
              ...
              <div className="" style={{ width: "130px", borderBottom: "1px double black", paddingBottom: "2px" }}>
                <div className=" " style={{ borderBottom: "1px double black" }}>
                  <h4 className="text-black text-end font-semibold" style={{ fontSize: "12px" }}>
                    {formatNumber(selectedInvoice?.total_ttc) || 0}     Ar</h4>
                </div>
              </div>
            </div>
        ...
        <div className="mt-2">
          ...
          <p className="text-center uppercase font-semibold" style={{ fontSize: "12px" }}>{toWords.convert(isNaN(selectedInvoice?.total_ttc) ? 0 : (selectedInvoice?.total_ttc))} </p>
        </div>
        <div className="foot px-6 mt-0 flex justify-between">
          <div className="flex justify-between w-full">
            <div className="mode w-[340px]">
              <h4 style={{ fontSize: "13px" }}>Mode de paiement: <span className="ml-4 capitalize"> </span></h4>
              <h2 className="text-center mt-1 font-bold" style={{ fontSize: "13px" }}>Client</h2>
              <div className=" border  border-black p-2 w-[170px] mt-2">
                <input type="text" defaultValue={selectedInvoice?.mode_paiement} style={{ fontSize: "13px" }} className="outline-none text-center w-full" />
