...
export default function PrintMoreChecked({ MoreArticles, num }) {
  ...
  const total_HT = (MoreArticles.length != 0 ? ((MoreArticles.map((e: { prix_unitaire: number, total_HT: number }) => (e.total_HT))).reduce((acc, el) => acc + el)) : 0)
  const total_TCC = (MoreArticles.length != 0 ? ((MoreArticles.map((e: { prix_unitaire: number, total_TTC: number }) => (e.total_TTC))).reduce((acc, el) => acc + el)) : 0)
  const total_TVA = (MoreArticles.length != 0 ? ((MoreArticles.map((e: { prix_unitaire: number, TVA: number }) => (e.TVA))).reduce((acc, el) => acc + el)) : 0)
    ...
    <div className="view-invoices fixed  times left-0 top-0 z-50   w-full h-[870px] flex justify-center">
      ...
      <div className="cadre relative w-[560px] h-full bg-white pt-4">
        <div className="head">
          ...
          {
            (((MoreArticles[0]?.client.nif) != "") && ((MoreArticles[0]?.client.stat) != "")) &&
        ...
        <div className="client flex items-start justify-between px-6 pt-2">
          <div className="idt flex flex-col">
            <div className="list flex MoreArticles[0]s-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>Client   <span>:</span></h3>
              <input type="text" defaultValue={`${MoreArticles[0]?.client.firstName} ${MoreArticles[0].client.name}`}   className="outline-none text-sm" />
            </div>
            <div className="list flex MoreArticles[0]s-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>Adresse   <span>:</span></h3>
              <input type="text" defaultValue={MoreArticles[0]?.client.adresse} className="outline-none text-sm w-64" />
            </div>
            <div className="list flex MoreArticles[0]s-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>Téléphone   <span>:</span></h3>
              <input type="text" defaultValue={MoreArticles[0]?.client.telephone} className="outline-none text-sm" />
            </div>
            <div className="list flex MoreArticles[0]s-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>NIF   <span>:</span></h3>
              <input type="text" defaultValue={MoreArticles[0]?.client.nif} className="outline-none text-sm" />
            </div>
            <div className="list flex MoreArticles[0]s-center gap-4">
              <h3 className="text-sm font-bold text-black  w-12 flex justify-between" style={{ fontSize: "9px" }}>STAT   <span>:</span></h3>
              <input type="text" defaultValue={MoreArticles[0]?.client.stat} className="outline-none text-sm" />
            </div>
          </div>
          <div className="fact">
            <h3 className="text-sm font-bold text-center" style={{ fontSize: "13px" }}>Date:  {(format(new Date(), "dd/MM/yyyy"))}</h3>
            <div className="numero-facture border-2 border-black p-2 w-[190px] mt-1">
              <h4 className="text-center font-semibold" style={{ fontSize: "13px" }}>Facture</h4>
              <input type="text" className="outline-none mt-1 text-sm text-center font-bold w-full" defaultValue={num} placeholder="N facture" style={{ fontSize: "12px" }} />
            </div>
          </div>
        </div>
        <div className="array px-6 mt-2">
          <table className="w-full text-xs border-b-2 border-black ">
            ...
            <tbody>
              {
                MoreArticles.map((articles, i) => (

                  <tr key={i}>
                    <td className="border h-3 uppercase border-l-2  border-black text-xs font-bold" style={{fontSize:"11px"}}>{articles?.stock.designation}</td>
                    <td className="border h-3 border-black text-center w-4 font-bold"  style={{fontSize:"11px"}}>{formatNumber(articles.quantite)}</td>
                    <td className="border h-3 text-end pr-1 border-black w-20 text-nowrap font-bold" style={{fontSize:"11px"}}>
              ...
              {
                [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 10, 11, 12, 13].slice(MoreArticles.length).map((e, i) => (
        ...
        <div className="mt-2">
          ...
          <p className="text-center uppercase font-semibold" style={{ fontSize: "12px" }}>{toWords.convert( total_TVA != 0 ? total_TCC : total_HT)} </p>
        </div>
        <div className="foot px-6 mt-4">

          <div className="flex justify-between w-full">
            <div className="mode w-[340px]">
              <h4 style={{ fontSize: "13px" }} className='font-semibold ml-10'  >Mode de paiement: <span className="ml-4 capitalize"> {MoreArticles[0]?.mode_paiement}  </span></h4>
